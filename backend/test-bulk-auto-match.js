...
async function testBulkAutoMatch() {
  console.log('Testing bulk auto-matching for existing receipts...\n');
    ...
    receipts.forEach(receipt => {
      ...
      db.all(transactionQuery, [], (err, transactions) => {
        ...
        if (matches.length > 0 && matches[0].confidence >= 60) {
          ...
          console.log(`✅ Found match: ${bestMatch.transaction.description} | $${bestMatch.transaction.amount} | Confidence: ${bestMatch.confidence}%`);
          ...
          `, [
            ...
            if (err) {
              ...
              console.log(`✅ Created match for receipt ${receipt.id}`);
              matchedCount++;
            }
          });
        } else {
        ...
        // Check if we're done
        if (processedCount === receipts.length) {
          console.log(`\n=== Summary ===`);
          console.log(`Processed: ${processedCount} receipts`);
          console.log(`Matched: ${matchedCount} receipts`);
